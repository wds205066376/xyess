...
public class AddressServiceImpl implements AddressService {
    ...
    public boolean updateAddress(AddressModel addressModel) {
        if (addressModel.getId() == null || addressModel.getUserId() == null) {
            throw new IllegalArgumentException("地址ID和用户ID不能为空");
        }
        AddressModel dbModel = addressDao.selectByPrimaryKey(addressModel.getId());
        if (dbModel == null || !dbModel.getUserId().equals(addressModel.getUserId())) {
            throw new IllegalArgumentException("无权修改该地址");
        }
        if (addressModel.getDefaultFlag()) {
            // 同新增地址时的逻辑
            AddressModel a = new AddressModel();
    ...
    public boolean deleteAddress(AddressModel addressModel) {
        ...
        AddressModel dbModel = addressDao.selectByPrimaryKey(addressModel.getId());
        if (dbModel == null || !dbModel.getUserId().equals(addressModel.getUserId())) {
            throw new IllegalArgumentException("无权删除该地址");
        }
